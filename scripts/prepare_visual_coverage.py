...
def build_plans(control: dict, review: dict) -> dict[str, dict]:
    ...
    for stage, batch, steps in (
        ...
        value.update(
            status="pending_compute_authorization",
            plan_id=name,
            run_name=name,
            hypothesis=(
                "Increase frame-zero scene coverage from 8 to 40 at fixed 256 updates "
                "and 1024 exposures; repetition allocation also changes."
            ),
            formal_training_claim=False,
            formal_training_authorized=False,
        )
        value["optimizer_smoke"].update(
            run_name=name,
            episodes=train,
            batch_size=batch,
            steps=steps,
            save_freq=steps,
            log_freq=min(16, steps),
        )
        value["preflight"].update(run_name=f"{name}-preflight", batch_size=batch)
        for feature in value["features"]:
            if feature["name"] == "fixed_frame_sampler":
                feature["sample_identities"] = [
                    {"episode": ep, "frame": 0} for ep in train
                ]
...
def prepare(output: Path) -> dict:
    ...
    stages = {
        name: save(output / f"{name}.json", plan)
        for name, plan in build_plans(control, review).items()
    }
    implementation = {
        **control["implementation_files"],
        **{
    ...
    registration = {
        "schema_version": 1,
        "protocol": review["evaluation"]["protocol_id"],
        "status": "draft_no_compute_authorization",
        "model_execution_authorized": False,
        "deadline_unix": None,
        "started_unix": None,
        "external_watchdog_verified": False,
        "review_plan": {"path": REVIEW, "sha256": digest(ROOT / REVIEW)},
        "implementation_files": implementation,
        "upstream_files": UPSTREAM,
        "stages": stages,
        "stage_order": [
            "doctor_and_static_checks",
            "benchmark",
            "preflight-b1",
            "preflight-b4",
            "smoke2",
            "smoke2_independent_reload",
            "control_reproduction",
            "main256_fresh_base",
            "frozen_and_optimizer_audit",
            "seal_checkpoints",
            "A_collect",
            "A_fresh_process_collect",
            "A_reload_compare",
            "B_collect",
            "B_fresh_process_collect",
            "B_reload_compare",
            "compare_arms",
        ],
        ...
        "pending": [
            "fresh GPU/compute authorization and shared deadline/watchdog",
        ...
        "arms": {
            "A": {
                "run_name": control["run_name"],
                "plan": {"path": CONTROL, "sha256": digest(ROOT / CONTROL)},
                "checkpoint_relative_to_root": prefix + "/pretrained_model",
                "checkpoint_files": files,
            },
            "B": {
                "run_name": build_plans(control, review)["main256"]["run_name"],
                "plan": stages["main256"],
                "checkpoint_relative_to_root": (
                    f"{control['parent_experiment']['experiment_id']}/smoke/"
                    "m2-smolvla450m-visual-coverage40-main256-001/"
                    "checkpoints/000256/pretrained_model"
                ),
                "checkpoint_files": None,
            },
        },
        "main_phase": "smoke",
        "B_initialization": "revision_pinned_base_model",
        "A_retrained": False,
        "old_optimizer_resumed": False,
        "sampler_exposures": 1024,
