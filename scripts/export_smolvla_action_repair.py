"""Export Faust while preserving and independently reloading its repaired action boundary."""
...
def _plan_path() -> Path:
    try:
        ...
        raise ValueError("Faust export requires an explicit --plan path.") from error
...
def main() -> int:
    ...
    cache_root = run_root / "compiler_cache" / f"faust-export-{file_sha256(plan_path)[:12]}"
