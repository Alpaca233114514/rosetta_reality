"""Train the plan-bound Way CUDA optimizer smoke on AutoDL."""
...
def main() -> None:
    if (
        os.environ.get("ROSETTA_VLA_STATE_ROBUSTNESS_CUDA_SMOKE_AUTHORIZED") != "1"
        or os.environ.get("ROSETTA_VLA_PHASE") != "performance_benchmark"
        or os.environ.get("ROSETTA_TORCH_DEVICE") != "cuda"
    ):
        raise PermissionError("Way CUDA optimizer smoke is not authorized.")
    import lerobot.scripts.lerobot_train as lerobot_train
    import run_smolvla_state_robustness_cuda_smoke as runner
    ...
    plan_raw = os.environ.get("ROSETTA_VLA_STATE_ROBUSTNESS_CUDA_PLAN_PATH")
    if not plan_raw:
        raise ValueError("Way CUDA training requires its plan path.")
    ...
    if (
        ...
        raise ValueError("The active optimizer process differs from Way CUDA smoke.")
