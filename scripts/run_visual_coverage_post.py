...
def verify_sample_observation(observed, expected, *, consumed, batch_size):
    if len(observed) != consumed + batch_size or observed != expected[: len(observed)]:
        raise ValueError(
            "Sampler observation differs from consumed plus one prefetched batch"
        )
...
def worker():
    ...
    try:
        ...
        assert (
            j.digest(loader.__file__)
            == "e08c5f5cd3f7a8b8aaece03ad51f1c37994e390faf47d368281ad49c31bc84d1"
        )
        assert (
            j.digest(trainer.__file__)
            == "4d15d283ea54583f552b32088db0b6c195250905ca6daf06d4670383790e2059"
        )
        observed = []
        ...
        synthetic = iter(
            DataLoaderShard(
                list(range(1040)), batch_size=4, sampler=Sampler(), num_workers=0
            )
        )
        consumed = []
        for _ in range(256):
        ...
        proof = verify_sample_observation(
            resources["sampled_indices"], expected, consumed=1024, batch_size=4
        )
        assert (
            resources["status"] == "passed" and len(resources["learning_rates"]) == 256
        )
        ...
        assert metric["step"] == 256 and all(
            math.isfinite(v) for v in metric["metrics"].values()
        )
        saved = j.load(cp / "pretrained_model/train_config.json")
        contract = j.load(OLD / "A-before-B-contract.json")
        control = (
            Path(os.environ["ROSETTA_CHECKPOINT_ROOT"])
            / contract["arms"]["A"]["checkpoint_relative_to_root"]
        )
        ...
        proof.update(
            status="passed",
            source_training_code="e11c7aaddc8c1eab84365d399ef8696cb5d0f83b",
            synthetic_accelerate_prefetch_verified=True,
            resources=j.evidence(OLD / "main256-resources.json"),
            update_audit=j.evidence(OLD / "B-update-audit.json"),
            additional_optimizer_steps=0,
        )
        j.save(j.JOB / "accepted-B_training_integrity.json", proof)
        watch = j.load(j.JOB / "watchdog.json")
        contract.update(
            started_unix=watch["started_unix"], deadline_unix=watch["deadline_unix"]
        )
        contract["implementation_files"]["scripts/run_visual_coverage_post.py"] = (
            j.digest(j.SELF)
        )
        contract["prerequisite_evidence"]["B_training_integrity"] = j.evidence(
            j.JOB / "accepted-B_training_integrity.json"
        )
        contract["arms"]["B"].update(
            plan={
                "path": plan.relative_to(j.ROOT).as_posix(),
                "sha256": j.digest(plan),
            },
            run_name=name,
            checkpoint_relative_to_root=(cp / "pretrained_model")
            .relative_to(Path(os.environ["ROSETTA_CHECKPOINT_ROOT"]))
            .as_posix(),
        )
        for arm in ("A", "B"):
            contract["arms"][arm]["checkpoint_files"] = j.inventory(
                Path(os.environ["ROSETTA_CHECKPOINT_ROOT"])
                / contract["arms"][arm]["checkpoint_relative_to_root"]
            )
        ...
        def run(name, args, heavy=False):
            ...
            if heavy:
                command = [
                    ...
                    "wrap",
                    str(j.JOB / f"{name}-resources.json"),
                    *command[1:],
                ]
            with (j.JOB / f"{name}.log").open("x") as stream:
                subprocess.run(
                    ...
                    timeout=max(1, watch["deadline_unix"] - time.time()),
                )
        ...
        for arm in ("A", "B"):
            ...
            run(
                ...
                [
                    "reload",
                    "--first",
                    str(j.JOB / f"{arm}-first"),
                    "--second",
                    str(j.JOB / f"{arm}-reload"),
                    "--output",
                    str(j.JOB / f"{arm}-reload-check.json"),
                ],
            )
        run(
            "compare-arms",
...
if __name__ == "__main__":
    if sys.argv[1] == "prepare":
        ...
        shutil.copytree(
            source / "runs/visual-native-small-001",
            j.ROOT / "runs/visual-native-small-001",
        )
        j.save(
            j.JOB / "registration.json",
            {
                "job_sources": {str(j.SELF.relative_to(j.ROOT)): j.digest(j.SELF)},
                "optimizer_steps": 0,
                "budget_seconds": 1800,
            },
        )
