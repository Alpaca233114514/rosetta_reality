...
class ImageValueComponents:
    ...
    def __init__(self, policy, donor_file, condition, calibration_dir):
        ...
        if condition != "base1280":
            ...
            if (
                ...
                != native_result["image_value_component"]["calibration_sha256"]
            ):
    ...
    def hook(self, layer):
        def apply(module, inputs, output):
            ...
            full_x, full_output = inputs[0], output
            if tuple(full_x.shape) != (1, 241, 320) or full_output.shape != full_x.shape:
                raise ValueError("Native V prefix shape changed")
            x, output = full_x[:, :64], full_output[:, :64]
            if self.layout is None:
                raise ValueError("Missing registered full-prefix layout")
            ...
            if key not in self.cache:
                donor = functional.linear(full_x, self.donors[layer])[:, :64]
            ...
            if self.mode == "native":
                return full_output
            global_mean, token_pattern = self.means.get(layer, (None, None))
            replacement = intervention_output(
                output, donor, global_mean, token_pattern, self.mode, self.sign
            )
            result = full_output.clone()
            result[:, :64] = replacement
            return result
    ...
    def finish(self, output):
        ...
        report = {
            ...
            "scope": "real_image_tokens_0_64",
            "untouched_positions": [64, 241],
            "layout": self.layout,
            "mode": self.mode,
