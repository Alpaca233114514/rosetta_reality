...
from scripts.hestia_scene_kv import validate_protocol  # noqa: E402
...
def supervise(args, template):
    ...
    try:
        ...
        for condition in registration["allowed_conditions"]:
            ...
            with (job / f"condition-{condition}.log").open("xb") as stream:
                child = subprocess.Popen(
                    [
                        sys.executable,
                        "scripts/diagnose_hestia_scene_kv.py",
                        "--template",
                        str(args.template),
                        "--permit",
                        str(job / "permit.json"),
        ...
        if sum(v["bytes"] for v in files.values()) > 256 * 1024**2:
