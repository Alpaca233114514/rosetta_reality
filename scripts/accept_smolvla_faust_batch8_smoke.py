"""Accept the plan-bound Faust batch-8 optimizer smoke without loading weights."""
...
def _trackio_evidence(
    ...
    try:
        ...
        connection.close()
    if len(configs) != 1:
        raise ValueError("Faust smoke must have exactly one durable Trackio config.")
    ...
    if any(config.get(key) != value for key, value in expected_config.items()):
        raise ValueError("Faust smoke Trackio config differs from its plan.")
    if {str(row[0]) for row in rows} != {run_id}:
        raise ValueError("Faust smoke Trackio rows have an ambiguous identity.")
    ...
    for _, step, raw in rows:
        ...
        if "train/loss" in metrics:
            ...
            if step_number in train:
                raise ValueError("Faust smoke contains duplicate train metrics.")
            for key in (
                "train/loss",
                "train/grad_norm",
                "train/lr",
    ...
    if set(train) != expected_steps or checkpoint_steps != expected_steps:
        raise ValueError("Faust smoke lacks a registered metric or checkpoint step.")
    return {
...
def _checkpoint_evidence(
    ...
    for step in range(1, int(smoke["steps"]) + 1):
        ...
        if any(not path.is_file() or path.stat().st_size <= 0 for path in required):
            raise FileNotFoundError("Faust smoke checkpoint is incomplete.")
        ...
        if (
            ...
            raise ValueError("Faust smoke checkpoint identity differs from the plan.")
        evidence.append(
            {
                "step": step,
...
def main() -> int:
    ...
    if trackio["peak_xpu_within_guardrail"] is not True:
        raise MemoryError("Faust batch-8 smoke exceeded its XPU guardrail.")
    contract_path = REPOSITORY_ROOT / str(experiment["action_contract"]["derived"])
    report = {
        "schema_version": 1,
        "status": "passed",
        "stage": "smolvla_action_repair_batch8_optimizer_smoke_acceptance",
    ...
    if destination.exists():
        raise FileExistsError("Faust batch-8 smoke acceptance is create-only.")
