...
if [[ "$mode" == format ]]; then
 ...
 mkdir -p runs/iris-gate-preparation-20260913-003
 mounts+=(--mount "type=bind,source=$repo/runs/iris-gate-preparation-20260913-003,target=/workspace/runs/iris-gate-preparation-20260913-003")
fi
...
if [[ "$MODE" == format ]]; then
 ...
 python scripts/run_iris_gate.py prepare --template runs/iris-gate-preparation-20260913-003/template.json
else
