...
from scripts.hestia_v_layer import CONDITIONS, keys_for

IMAGE = "sha256:fb3c1bbda42881fac9b7725d3acb436119934f0c60af29747339d5951c3039da"
ROOT = Path("runs/hestia-v-layer-recovered-20260912-001/verified")
OUTPUT = Path("runs/hestia-v-layer-analysis-20260912-001/verified")
...
def causal(values, kind):
    if kind == "all_v":
        forward, reverse = "base1280_v640", "base640_v1280"
    else:
        forward, reverse = f"base1280_v{kind}_640", f"base640_v{kind}_1280"
...
def main():
    ...
    receipt = read_json("runs/hestia-v-layer-receiver-20260912-001/receipt.json")
    ...
    template = read_json("reports/training/m2-smolvla-hestia-v-layer-template-2026-09-12.json")
        ...
        for window, slots in (("full", slice(0, 50)), ("first", slice(0, 1))):
            ...
            for metric in errors[CONDITIONS[0]]:
                ...
                out[metric] = {
                    ...
                    "causal": {
                        str(kind): causal(values, kind) for kind in (*range(1, 16, 2), "all_v")
                    },
                    "group_minus_sum_individual_effects": {
                        "forward_recovery": summarize(
                            ...
                            - sum(
                                values["base1280"] - values[f"base1280_v{i}_640"]
                        ...
                        "reverse_damage": summarize(
                            ...
                            - values["base640"]
                            - sum(
                                values[f"base640_v{i}_1280"] - values["base640"]
                                for i in range(1, 16, 2)
                            )
                        ),
                    },
        ...
        for window, (start, stop) in windows.items():
            ...
            for arm in ("left", "right"):
                for joint, suffix in ((True, "joint"), (False, "gripper")):
                    ...
                    changes = {}
                    for name in (*CONDITIONS[2:], "base1280_v640", "base640_v1280"):
