...
def main():
    ...
    episodes = experiment["dataset"]["train_episodes"] + experiment["dataset"]["validation_episodes"]
    assert len(episodes) == 45 and not set(episodes) & set(experiment["dataset"]["test_episodes"])
    smoke = plan["optimizer_smoke"]
    source = Path(os.environ["ROSETTA_CHECKPOINT_ROOT"]) / experiment["experiment_id"] / "smoke" / smoke["run_name"] / "checkpoints" / f"{smoke['steps']:06d}" / "pretrained_model"
    ...
    meta = n.LeRobotDatasetMetadata(data_cfg.repo_id, root=context["root"], revision=data_cfg.revision)
    dataset = n.LeRobotDataset(data_cfg.repo_id, root=context["root"], episodes=episodes, revision=data_cfg.revision,
                              delta_timestamps=n.resolve_delta_timestamps(cfg, meta), download_videos=False, return_uint8=True)
    dataset.meta.stats.update({k: {s: np.asarray(v) for s, v in values.items()} for k, values in norm["effective_stats"].items()})
    torch.manual_seed(experiment["seed"])
    policy = n.make_policy(cfg=cfg, ds_meta=dataset.meta, rename_map=experiment["dataset"]["rename_map"])
    pre, post = n._make_processors(SimpleNamespace(policy=cfg, rename_map=experiment["dataset"]["rename_map"]), policy, dataset, torch.device("cuda"))
    physical = n.load_action_contract(physical_path)
    n.ensure_smolvla_action_boundary(pre, post, physical, n.load_smolvla_action_space(experiment),
                                    action_contract_sha256=physical_sha, upstream_revision=experiment["upstream"]["revision"])
    ...
    starts = dict(zip(dataset.meta.episodes["episode_index"], dataset.meta.episodes["dataset_from_index"], strict=True))
    batches, records = [], []
    ...
    for ep in episodes:
        ...
        record = {"episode": ep, "frame": 0, "raw_image_sha256": digest(raw[camera]),
                  "raw_state_sha256": digest(raw["observation.state"]), "target_sha256": digest(raw["action"]),
                  "timestamp": float(sample["timestamp"]), "task": sample["task"]}
        ...
        fields = [state, batch["observation.language.tokens"], batch["observation.language.attention_mask"], *masks, *images[1:]]
        record["nonvisual_sha256"] = hashlib.sha256("|".join(digest(x) for x in fields).encode()).hexdigest()
    ...
    if mode == "samples":
        ...
        train = n.LeRobotDataset(data_cfg.repo_id, root=view, episodes=active, revision=data_cfg.revision,
                                delta_timestamps=n.resolve_delta_timestamps(cfg, meta), download_videos=False, return_uint8=True)
        indices = resolve_visual_sample_indices([(ep, 0) for ep in active], train.meta.episodes["dataset_from_index"],
                                                train.meta.episodes["dataset_to_index"], active, train.absolute_to_relative_idx)
        for ep, index in zip(active, indices, strict=True):
            row = train[index]
            assert int(row["episode_index"]) == ep and int(row["frame_index"]) == 0
        save(output, {"status": "passed", "records": records, "actual_train_view_indices": indices,
                      "actual_train_episodes": active, "action_contract_sha256": physical_sha,
                      "normalization_sha256": n.file_sha256(norm_path), "model_source_sha256": n.file_sha256(source / "model.safetensors"),
                      "hidden_test_loaded": False, "optimizer_steps": 0})
    elif mode == "predict":
        ...
        for seed in (None, 20260905, 20260906, 20260907):
            noise = torch.zeros((1, 50, 32)) if seed is None else torch.randn((1, 50, 32), generator=torch.Generator().manual_seed(seed))
        ...
        save(str(output) + ".json", {"status": "passed", "pid": os.getpid(), "records": records,
                                     "checkpoint_sha256": n.file_sha256(source / "model.safetensors"), "optimizer_steps": 0})
    else:
