"""Select one candidate checkpoint per registered protocol.
...
``smolvla_vcdropout_validate``; ties break toward the earlier checkpoint. The
...
are not same-tree comparisons and never gate selection.

Usage:
    python scripts/select_smolvla_vcdropout_checkpoint.py --plan <candidate-plan> \
...
def main() -> int:
    ...
    labels = [f"{protocol.VCD_VALIDATION_PREFIX}-base.json"] + [
    ...
    decision = {
        "schema_version": 1,
        "status": "selected",
        "stage": "smolvla_vcdropout_validation_only_selection",
        "experiment_id": protocol.EXPERIMENT_ID,
        "plan_id": plan_id,
        "plan_sha256": file_sha256(plan_path),
        "role": "treatment_vs_immutable_zen_uniform_control",
        ...
        "read_only_context_controls": {
            "note": (
                ...
                "run; the executable offset-250 gradient gate, not offline MAE, "
                "decides the axis."
            ),
            "zen_uniform_control_first_action_mae": 0.021572770214905695,
            "zen_firstaction_offline_first_action_mae": 0.022150604739519103,
    ...
    destination = (
        ...
        / "selection"
        / f"{protocol.VCD_RUN_NAME}-selection.json"
    )
