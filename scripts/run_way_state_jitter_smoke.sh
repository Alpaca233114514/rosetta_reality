...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/way-state-jitter-smoke-001.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/way-state-jitter-smoke-001.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Way state-jitter smoke status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=way-state-jitter-smoke-001 \
    scripts/run_m2_container.sh vla-xpu \
    python -u scripts/run_smolvla_state_robustness_smoke.py \
    --plan /workspace/configs/vla/smolvla_450m_aloha_insertion_way_state_jitter_smoke_001.yaml \
    >"$LOG_PATH" 2>&1
exit_code="$?"
set -e
