...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/faust-b8-gate3-002.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/faust-b8-gate3-002.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Faust Gate 3 status or log already exists\n' >&2; exit 3; }
