"""Guarded same-device CUDA observations of existing Hestia checkpoints; no training."""
...
def collect(plan, output):
        ...
        for ni, noise in enumerate(noises):
            ...
            for bi, batch in enumerate(batches):
                ...
                if plan["step"] == 1280 and ni == 0 and bi == 0:
                    first = difference(
                        ps[-1],
                        old["normalized_predictions"][0, 0],
    ...
    if plan["step"] == 1280:
        for key, atol in (
            ...
            control[key] = difference(arrays[key], old[key], atol, plan["rtol"])
        deltas = []
        for key, target in (
    ...
    if plan["step"] == 1280 and not control["passed"]:
        raise ValueError("Full same-device parity failed; quarters remain stopped")
    print(
        json.dumps(
            {
                "status": "passed",
...
def validate_permit(template, permit, step, now):
    if step not in (1280, 320, 640, 960):
        raise ValueError("Unregistered checkpoint step")
    if (
    ...
    if permit.get("allowed_steps") != [1280, 320, 640, 960]:
        raise ValueError("Registered step order differs")
...
def main():
    ...
    validate_permit(file_hash(args.template), permit, args.step, time.time())
    actual_ticks = (
        Path(f"/proc/{permit['watchdog_pid']}/stat").read_text().rsplit(")", 1)[1].split()[19]
    )
    ...
    if (
        ...
        raise ValueError("Registered AutoDL profile required")
    ...
    for name, sha in plan["upstream_files"].items():
        if file_hash(installed / name) != sha:
            raise ValueError("Pinned upstream differs")
    job = Path(plan["output"])
    if args.step != 1280:
        control = json.loads((job / "001280/result.json").read_text())
        if (
            control.get("step") != 1280
            or control.get("same_device_control", {}).get("passed") is not True
        ):
            raise ValueError("Exact final-checkpoint control must pass first")
        if control["array_sha256"] != file_hash(job / "001280/arrays.npz"):
            raise ValueError("Control arrays changed")
    checkpoint_root = Path(os.environ["ROSETTA_CHECKPOINT_ROOT"]).resolve(strict=True)
    source = checkpoint_root / plan["checkpoint_prefix"] / f"{args.step:06d}" / "pretrained_model"
    if not source.resolve(strict=True).is_relative_to(checkpoint_root):
        raise ValueError("Checkpoint path escaped")
    for name, sha in plan["checkpoints"][str(args.step)].items():
        p = source / name
        if (
            p.is_symlink()
            or not p.resolve(strict=True).is_relative_to(source.resolve())
            or file_hash(p) != sha
        ):
    ...
    # Heavy/runtime imports remain behind the explicit permit and provenance gates.
    import torch
    ...
    if (
        ...
        raise ValueError("Original CUDA runtime versions required")
    if (
