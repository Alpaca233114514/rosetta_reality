"""Export the selected vfunfreeze checkpoint as a gate-ready deploy artifact.

The artifact layout matches what the frozen simulation-gate engine loads
(``smolvla_sim_gate._load_artifact``) and what the corrected frame-0 paired
alignment probe consumes: ``pretrained_model/``, ``config.json`` (dataset
features/fps, rename map, action-space identity, mixed precision, the
training-only treatment identity), ``normalization.json`` (train-only
statistics), ``action_contract.json`` and a ``manifest.json`` whose
``reload.exact_tensor_equality`` is proven by re-running the fixed-validation
engine against the exported weights in a fresh process and comparing the
...
def _reload_validation(
    ...
    environment = {
        **os.environ,
        protocol.VFU_RELOAD_SOURCE_ENV: str(artifact_pretrained),
        protocol.VFU_VALIDATION_PREFIX_ENV: prefix_override,
    }
    ...
    return (
        ...
        / "validation"
        / f"{prefix_override}-step-{step:06d}.json"
    )
...
def main() -> int:
    ...
    if selection.get("plan_sha256") != file_sha256(plan_path):
        raise ValueError("Selection report belongs to a different candidate plan.")
    ...
    artifact_id = f"{protocol.VFU_RUN_NAME}-step{step:04d}-deploy-001"
    ...
    artifact_config = {
        ...
        "training_only_treatment": {
            "vision_front_end_profile": protocol.VFU_CONTRACT["profile"],
            "vision_front_end_scope": list(protocol.VFU_CONTRACT["scope"]),
            "vision_front_end_language_model_frozen": True,
            "vision_front_end_training_only": True,
            "deployment_vision_front_end_scope": False,
        },
    ...
    preflight_report = (
        run_root / protocol.EXPERIMENT_ID / "preflight" / f"{protocol.VFU_PREFLIGHT_RUN_NAME}.json"
    )
    if not preflight_report.is_file():
        raise FileNotFoundError(
            f"Candidate preflight report missing: {preflight_report.name}"
        )
    checkpoint_report = (
        run_root
        / protocol.EXPERIMENT_ID
        / "validation"
        / f"{protocol.VFU_VALIDATION_PREFIX}-step-{step:06d}.json"
    )
    if not checkpoint_report.is_file():
        raise FileNotFoundError(f"Checkpoint validation report missing: {checkpoint_report.name}")
    reload_prefix = f"{protocol.VFU_VALIDATION_PREFIX}-reload"
    validation_dir = run_root / protocol.EXPERIMENT_ID / "validation"
    generation = 1
    while (validation_dir / f"{reload_prefix}-step-{step:06d}.json").is_file():
        generation += 1
        reload_prefix = f"{protocol.VFU_VALIDATION_PREFIX}-reload{generation}"
    ...
    manifest = {
        ...
        "stage": "smolvla_vfunfreeze_export",
        "artifact_id": artifact_id,
        "experiment_id": protocol.EXPERIMENT_ID,
        "plan_id": plan_id,
