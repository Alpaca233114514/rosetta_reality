"""Run fixed Aster modality ablations through its registered action boundary."""
...
def main() -> int:
    """Bind the generic read-only diagnostic to Aster's frozen plan and checkpoint."""

    aster_evaluator._validate_delegated_evaluator()
    diagnostic.DEFAULT_PLAN = formal_runner.DEFAULT_PLAN
    ...
    evaluator._checkpoint_source = aster_evaluator._checkpoint_source
    evaluator._validate_checkpoint_statistics = (
        aster_evaluator._validate_checkpoint_statistics
    )
    ...
    def create_json(path: Path, payload: dict[str, Any]) -> None:
        ...
        payload["temporal_loss_profile"] = plan["loss_contract"]["profile"]
        payload["temporal_loss_normalization"] = plan["loss_contract"][
            "normalization"
        ]
        payload["aster_diagnostic_script_sha256"] = file_sha256(Path(__file__))
