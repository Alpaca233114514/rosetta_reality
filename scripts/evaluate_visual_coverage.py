"""Collect and compare the registered full-chunk visual coverage evidence.

Model execution requires a separately authorized, sealed execution contract.
The template produced by prepare_visual_coverage.py deliberately cannot launch.
...
from rosetta_reality.vla.visual_coverage import (  # noqa: E402
    PROTOCOL,
    action_groups,
    array_hash,
    compare_arms,
...
def check_execution(contract: dict) -> None:
    if (contract.get("protocol") != PROTOCOL or contract.get("status") != "authorized_sealed"
            or contract.get("model_execution_authorized") is not True):
        raise ValueError("A fresh authorized execution contract is required; the draft cannot run.")
    deadline = contract.get("deadline_unix")
    if not isinstance(deadline, (int, float)) or not time.time() < deadline:
        raise ValueError("The shared execution deadline is absent or expired.")
    if os.environ.get("HF_HUB_OFFLINE") != "1" or os.environ.get("HF_DATASETS_OFFLINE") != "1":
        raise ValueError("Offline execution is required.")
    for name, digest in contract["implementation_files"].items():
        if file_hash(repository_file(name)) != digest:
            raise ValueError(f"Implementation identity drift: {name}.")
    installed = Path(importlib.metadata.distribution("lerobot").locate_file("lerobot"))
    for name, digest in contract["upstream_files"].items():
        relative = Path(name)
        if relative.is_absolute() or ".." in relative.parts or file_hash(installed / relative) != digest:
            raise ValueError("Installed LeRobot source identity drift.")
    review = repository_file(contract["review_plan"]["path"])
    if file_hash(review) != contract["review_plan"]["sha256"]:
        raise ValueError("Review plan identity drift.")
    for name, item in contract["prerequisite_evidence"].items():
        if not isinstance(item, dict) or file_hash(repository_file(item["path"])) != item["sha256"]:
            raise ValueError(f"Missing or changed prerequisite evidence: {name}.")
    required = {"environment", "sample_contract", "resource_preflight", "control_reproduction"}
    if not required <= set(contract["prerequisite_evidence"]):
...
def collect(contract_path: Path, arm: str, output: Path) -> dict:
    ...
    def budget_check():
        ...
        if (torch.cuda.max_memory_allocated() > limits["cuda_allocated_bytes"]
    ...
    data = review["data"]
    views = {"train8": data["train40"][:8], "train40": data["train40"],
    ...
    source = Path(os.environ["ROSETTA_CHECKPOINT_ROOT"]) / checkpoint_relative
    consumed = identity["checkpoint_files"]
    if not isinstance(consumed, dict) or not {"model.safetensors", "config.json", "train_config.json"} <= set(consumed):
    ...
    expected_episodes = views["train8"] if arm == "A" else views["train40"]
    if (saved["dataset"]["episodes"] != expected_episodes or saved["steps"] != 256
            or saved["batch_size"] != 4 or saved["seed"] != 20260809
            or saved["job_name"] != identity["run_name"]):
        raise ValueError("Saved training configuration differs from the arm.")
    if arm == "B" and not {"two_step_smoke_reload", "B_training_integrity"} <= set(contract["prerequisite_evidence"]):
        raise ValueError("Candidate smoke/reload/training integrity evidence is missing.")
    ...
    if (cfg.num_steps != 10 or cfg.chunk_size != 50 or cfg.max_action_dim != 32
            or cfg.adapt_to_pi_aloha or cfg.use_amp):
    ...
    meta = native.LeRobotDatasetMetadata(data_cfg.repo_id, root=context["root"], revision=data_cfg.revision)
    dataset = native.LeRobotDataset(data_cfg.repo_id, root=context["root"], episodes=episodes,
        revision=data_cfg.revision, delta_timestamps=native.resolve_delta_timestamps(cfg, meta),
    ...
    policy = native.make_policy(cfg=cfg, ds_meta=dataset.meta,
                                rename_map=experiment["dataset"]["rename_map"])
    pre, post = native._make_processors(SimpleNamespace(policy=cfg,
        rename_map=experiment["dataset"]["rename_map"]), policy, dataset, torch.device("cuda"))
    native.ensure_smolvla_action_boundary(pre, post, physical,
        native.load_smolvla_action_space(experiment), action_contract_sha256=physical_sha,
    ...
    decoder = decoders[0]
    dimensions = [{"name": d.name, "unit": d.unit, "encoding": d.encoding,
                   "minimum": d.minimum, "maximum": d.maximum} for d in physical.dimensions]
    ...
    def tensor_digest(value):
        return hashlib.sha256(str(value.dtype).encode() + array_hash(cpu(value)).encode()).hexdigest()
    ...
    actual_camera = experiment["dataset"]["rename_map"].get(dataset.meta.camera_keys[0], dataset.meta.camera_keys[0])
    required_keys = {"observation.state", "observation.language.tokens",
                     "observation.language.attention_mask", actual_camera}
    for ep in episodes:
        budget_check()
    ...
    noises = [torch.zeros((1, 50, 32)) if seed is None else torch.randn((1, 50, 32),
    ...
    def traced_sample(images, masks, tokens, attention, state, noise=None, **kwargs):
        ...
        if (not bool(masks[0].all()) or any(bool(x.any()) for x in masks[1:])
        ...
        traces.append((tensor_digest(images[0]), hashlib.sha256(
            "|".join(tensor_digest(x) for x in fields).encode()).hexdigest(), tensor_digest(noise)))
    ...
    try:
        ...
        for name, value in {"current_normalized": ps, "current_standard": ss,
                            "current_internal_grippers": gs}.items():
            with (partial / f"{name}.npy").open("xb") as stream:
                np.save(stream, np.asarray(value), allow_pickle=False)
        write_json(partial / "status.json", {"status": "incomplete_not_scored",
            "completed_noise_conditions": len(predictions), "current_predictions": len(ps),
            "hidden_test_loaded": False, "m2_complete": False})
        raise
    finally:
    ...
    arrays = {"normalized_predictions": np.asarray(predictions), "standard_predictions": np.asarray(decoded),
              "internal_grippers": np.asarray(internal), "normalized_targets": np.asarray(targets),
              "standard_targets": np.asarray(standards), "valid_mask": np.ones((45, 50, 14), dtype=bool),
              "noise": np.stack([n.numpy() for n in noises])}
    metadata = {"protocol": PROTOCOL, "arm": arm, "episodes": episodes, "views": views,
        "hidden_episodes": data["sealed_hidden5"], "hidden_test_loaded": False, "target_conditioning": False,
        "native_denoising_steps": 10, "model_mode": "eval_inference", "chunk_length": 50,
        "max_action_dim": 32, "dimensions": dimensions, "noise_conditions": [None, 20260905, 20260906, 20260907],
        "noise_hashes": [array_hash(n) for n in arrays["noise"]],
        "nonvisual_hashes": [t[1] for t in traces[:45]], "image_hashes": [t[0] for t in traces[:45]],
        "raw_image_hashes": raw_images, "native_prediction_dtypes": value_dtypes,
        "common_identity": {"execution_contract_sha256": file_hash(contract_path),
            "physical_contract_sha256": physical_sha, "normalization_sha256": file_hash(norm_path),
            "noise_generation": "cpu_float32_then_cuda_no_cast", "inference_autocast": "bf16"},
...
def main() -> None:
    ...
    collect_parser.add_argument("--arm", choices=("A", "B"), required=True)
    ...
    if args.mode == "collect":
        try:
            ...
            write_json(failure, {"status": "failed", "error_type": type(exc).__name__,
                "optimizer_steps": 0, "task_success": "not measured", "m2_complete": False})
            raise
    else:
