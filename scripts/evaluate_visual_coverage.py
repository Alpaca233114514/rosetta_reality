...
def check_execution(contract: dict) -> None:
    if (
        ...
        raise ValueError(
            "A fresh authorized execution contract is required; the draft cannot run."
        )
    ...
    if (
        not isinstance(contract.get("started_unix"), (int, float))
        or not 0 < deadline - contract["started_unix"] <= 1800
        or not contract["started_unix"] <= time.time()
        or contract.get("external_watchdog_verified") is not True
    ):
        raise ValueError(
            "The shared budget must include a verified external deadline watchdog."
        )
    for key, maximum in {
        ...
        limit = contract.get("resource_limits", {}).get(key)
        if type(limit) is not int or not 0 < limit <= maximum:
            raise ValueError(
                "The resource contract is missing or exceeds the registered limit."
            )
    if (
    ...
    required_sources = {
        "scripts/evaluate_visual_coverage.py",
        "scripts/prepare_visual_coverage.py",
    ...
    if not required_sources <= set(contract["implementation_files"]):
        raise ValueError("Evaluator source identity is incomplete.")
    ...
    if set(contract["upstream_files"]) != {
        "policies/smolvla/modeling_smolvla.py",
        "datasets/sampler.py",
        "optim/schedulers.py",
    }:
        raise ValueError("Native source identity is incomplete.")
    for name, digest in contract["upstream_files"].items():
        relative = Path(name)
        if (
            relative.is_absolute()
            or ".." in relative.parts
            or file_hash(installed / relative) != digest
        ):
    ...
    for name, item in contract["prerequisite_evidence"].items():
        if (
            ...
            or file_hash(repository_file(item["path"])) != item["sha256"]
        ):
...
def collect(contract_path: Path, arm: str, output: Path) -> dict:
    ...
    norm_path, _, _ = native._validate_normalization(
        plan, experiment, parent, physical_sha
    )
    ...
    for name, digest in consumed.items():
        ...
        if (
            relative.is_absolute()
            or ".." in relative.parts
            or file_hash(source / name) != digest
        ):
    ...
    if (
        saved["dataset"]["episodes"] != expected_episodes
        or saved["steps"] != 256
        or saved["batch_size"] != 4
        or saved["seed"] != 20260809
        or saved["job_name"] != identity["run_name"]
    ):
        raise ValueError("Saved training configuration differs from the arm.")
    if (
        saved["optimizer"] != plan["training"]["optimizer"]
        or saved["scheduler"] != plan["training"]["scheduler"]
    ):
        raise ValueError(
            "Saved optimizer/scheduler recipe differs from the registered native recipe."
        )
    for key, expected in {
        ...
        if saved["policy"].get(key) is not expected:
            raise ValueError("Saved policy freeze/precision contract changed.")
    if arm == "B" and not {"two_step_smoke_reload", "B_training_integrity"} <= set(
        contract["prerequisite_evidence"]
    ):
        raise ValueError(
            "Candidate smoke/reload/training integrity evidence is missing."
        )
    ...
    if (
        ...
        raise ValueError(
            "Native inference shape/precision/representation contract changed."
        )
    ...
    decoders = [
        step for step in post.steps if isinstance(step, PiAlohaPostprocessorStep)
    ]
    if len(decoders) != 1:
    ...
    for ep in episodes:
        ...
        if [key for key in policy.config.image_features if key in batch] != [
            actual_camera
        ]:
            raise ValueError(
                "Actual image feature identity differs from the registered camera."
            )
        if batch["observation.state"].shape[-1] != 14:
            raise ValueError(
                "The actual state must have 14 dimensions; config shape is not proof."
            )
    ...
    def traced_sample(images, masks, tokens, attention, state, noise=None, **kwargs):
        ...
        if (
            not torch.isfinite(images[0]).all()
            or images[0].min() < -1
            or images[0].max() > 1
        ):
        ...
        traces.append(
            (
                ...
                hashlib.sha256(
                    "|".join(tensor_digest(x) for x in fields).encode()
                ).hexdigest(),
                tensor_digest(noise),
            )
        )
    ...
    try:
        for noise in noises:
            ...
            for batch in batches:
                ...
                gs.append(
                    cpu(decoder.last_model_action[0, :, groups["gripper_normalized"]])
                )
                ...
                if bool(illegal.any()):
                    raise ValueError(
                        "Predicted standard action required safety clipping."
                    )
    ...
    budget_check()
    if len(traces) != 180 or any(
        traces[i][:2] != traces[i % 45][:2] for i in range(180)
    ):
        raise ValueError("Native input trace count or per-noise conditioning drift.")
    for condition, noise in enumerate(noises):
        if any(
            t[2] != tensor_digest(noise)
            for t in traces[condition * 45 : (condition + 1) * 45]
        ):
...
def main() -> None:
    ...
    if args.mode == "collect":
        try:
            ...
            failure = (
                args.output.parent
                / f"{args.output.name}.failure-{uuid.uuid4().hex}.json"
            )
            write_json(
                failure,
                {
