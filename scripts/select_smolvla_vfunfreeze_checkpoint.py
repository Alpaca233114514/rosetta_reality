"""Select one vfunfreeze candidate checkpoint per registered protocol.
...
``smolvla_vfunfreeze_validate``; ties break toward the earlier checkpoint. The
base model report participates as candidate ``base`` but never wins by
construction unless every checkpoint regressed beyond it (recorded honestly).

Offline context controls are read-only values from completed identities; they
are not same-tree comparisons and never gate selection.  The registered
acceptance chain for this axis is the corrected frame-0 paired alignment probe
plus a separately registered Gate 3/4 comparison, never offline MAE alone.

Usage:
    python scripts/select_smolvla_vfunfreeze_checkpoint.py --plan <candidate-plan> \
...
def main() -> int:
    ...
    labels = [f"{protocol.VFU_VALIDATION_PREFIX}-base.json"] + [
    ...
    for label in labels:
        ...
        if report.get("formal_plan_sha256") != file_sha256(plan_path):
            raise ValueError(f"Validation report belongs to a different plan: {label}")
    ...
    decision = {
        "schema_version": 1,
        "status": "selected",
        "stage": "smolvla_vfunfreeze_validation_only_selection",
        "experiment_id": protocol.EXPERIMENT_ID,
        "plan_id": plan_id,
        "plan_sha256": file_sha256(plan_path),
        "role": "vision_front_end_treatment_vs_immutable_zen_uniform_control",
        ...
        "read_only_context_controls": {
            "note": (
                ...
                "The registered same-tree control is the immutable Zen-uniform "
                "run; acceptance for this axis is the corrected frame-0 paired "
                "alignment probe plus a separately registered Gate 3/4 "
                "comparison, never offline MAE alone."
            ),
            "zen_uniform_control_first_action_mae": protocol.CONTROL_ZEN_UNIFORM_FIRST_ACTION_MAE,
            "vcdropout_offline_first_action_mae": protocol.CONTROL_VCDROPOUT_FIRST_ACTION_MAE,
        },
    ...
    destination = (
        ...
        / "selection"
        / f"{protocol.VFU_RUN_NAME}-selection.json"
    )
