...
def validate_manifest(value):
    ...
    condition = r"base(?:640|1280)_(?:native|self|kmean|vmean|kvmean)"
    pattern = re.compile(
        r"(?:(?:registration|permit|cpu-check|doctor-check|normalization-check|worker-exited)\.json"
        rf"|condition-{condition}\.log|{condition}/"
        r"(?:intervention\.json|calibration-(?:train|dev)\.npz|arrays\.npz|result\.json"
        r"|failure\.json|first-control\.json|first-control-failure\.npz))"
    )
    total = 0
    for name, item in value["files"].items():
        ...
        total += item["bytes"]
    if total != value.get("total_bytes") or not 0 < total <= 256 * 1024**2:
...
def main():
    ...
    if args.archive.stat().st_size > 257 * 1024**2:
