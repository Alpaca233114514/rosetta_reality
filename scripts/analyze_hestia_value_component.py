...
from scripts.hestia_value_component import CONDITIONS, component_mode

IMAGE = "sha256:fb3c1bbda42881fac9b7725d3acb436119934f0c60af29747339d5951c3039da"
ROOT = Path("runs/hestia-value-component-recovered-20260912-001/verified")
OUTPUT = Path("runs/hestia-value-component-analysis-20260912-001/verified")
...
def main():
    ...
    receipt = read_json("runs/hestia-value-component-receiver-20260912-001/receipt.json")
    ...
    template = read_json("reports/training/m2-smolvla-hestia-value-component-template-2026-09-12.json")
    ...
    for name in CONDITIONS:
        ...
        require(intervention == r["value_component"], "Intervention report differs")
        ...
        require(name == "base1280" or intervention["reciprocal_delta_matches_calibration"], "Delta drift")
        ...
        require(intervention["calibration_sha256"] == file_hash(ROOT / "base1280/calibration.json"), "Calibration drift")
    ...
    for name in ("base1280", "base640", "base1280_v640", "base640_v1280"):
        ...
        with np.load(p, allow_pickle=False) as saved:
            ...
            if name in CONDITIONS[:2]:
                ...
                for key in ("normalized_predictions", "standard_predictions"):
                    require(np.array_equal(saved[key], arrays[current][key]), "Full activation control differs from prior parameter substitution")
                require(reports[current]["same_device_control"]["passed"], "Full positive control failed")
        ...
        for window, slots in (("full", slice(0, 50)), ("first", slice(0, 1))):
            ...
            for metric in errors[CONDITIONS[0]]:
                ...
                out[metric] = {
                    ...
                    "causal": {
                        str(kind): causal(values, kind) for kind in ("full", "global", "token", "scene")
                    },
