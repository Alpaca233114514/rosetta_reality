"""Train the plan-bound Way state-jitter optimizer smoke."""
...
def main() -> None:
    if (
        os.environ.get("ROSETTA_VLA_STATE_ROBUSTNESS_AUTHORIZED") != "1"
        or os.environ.get("ROSETTA_VLA_PHASE") != "performance_benchmark"
    ):
        raise PermissionError("Way state-jitter optimizer smoke is not authorized.")
    import lerobot.scripts.lerobot_train as lerobot_train
    import run_smolvla_state_robustness_smoke as runner
    ...
    plan_raw = os.environ.get("ROSETTA_VLA_STATE_ROBUSTNESS_PLAN_PATH")
    if not plan_raw:
        raise ValueError("Way state-jitter training requires a plan path.")
    plan_path = Path(plan_raw)
    plan, control, base_path, validated_experiment = runner._validate_plan(plan_path)
    if (
        base_path != config_path.resolve()
        or validated_experiment["experiment_id"] != experiment["experiment_id"]
        or os.environ.get("ROSETTA_VLA_STATE_ROBUSTNESS_PLAN_SHA256")
        != file_sha256(plan_path)
        or os.environ.get("ROSETTA_VLA_RUN_NAME") != plan["run_name"]
        or plan.get("hidden_test_loaded") is not False
    ):
        raise ValueError("The active optimizer process differs from Way smoke.")
    ...
    horizon_profile = horizon_profile_from_plan(
        control, int(experiment["model"]["policy"]["chunk_size"])
    )
