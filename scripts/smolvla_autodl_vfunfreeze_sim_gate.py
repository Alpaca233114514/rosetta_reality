"""Render the vfunfreeze simulation-gate plan and dispatch Gate 3 / Gate 4.

Faithful adaptation of the registered AutoDL Zen/Way gate chain for the
vision-front-end-unfreeze candidate: the frozen simulator engine
(``smolvla_sim_gate``) stays untouched. This wrapper swaps in the CUDA
online-policy class (a verbatim behavioral clone of the proven Zen class with
vfunfreeze identity strings), installs a fail-closed evidence-path resolver
over the run root, verifies the registered entry permit (the corrected
frame-0 paired alignment probe on the exported artifact), derives a
gate-facing selection record from the vfunfreeze selection decision, writes
an inventory backup record, and records sanitized provenance into every gate
report.

Arm suffix ``444`` (engine-enforced three digits), continuing the campaign
series after Zen ``411``/``422`` and vcdropout ``433``. Execution boundary:
AutoDL CUDA container instance, network disabled; Gate 3 must pass before
Gate 4; a pass or failure here closes the registered comparison either way.
"""
...
hypothesis: >-
  Closed-loop Gate evaluation for the preregistered vision-front-end-unfreeze
  candidate. Exactly the visual front end (vision_model + connector) joined
  the trainable set inside the trainer while the language model and every
  other training semantic stayed on the frozen Zen-uniform baseline; the
  registered fallback shape is batch 32 / 632 steps with per-module
  activation checkpointing. Environment seeds, inference noise seeds,
  action-contract projection, receding-horizon execution, simulator physics,
  collision semantics and acceptance thresholds are fixed and identical to
  the six failed identities.
single_axis_change:
  field: features.vision_front_end_unfreeze
  control: frozen_visual_front_end_zen_uniform_lineage
  candidate: vision_front_end_trainable_language_model_frozen
entry_permit:
  alignment_report: {alignment_report}
  alignment_report_sha256: {alignment_sha}
  criteria: zero_noise_paired_gain_positive_and_at_least_3_of_4_noise_settings_positive
prior_failure:
  report: {prior_report}
...
def _cuda_runtime() -> dict[str, Any]:
    if os.environ.get("ROSETTA_TORCH_DEVICE") != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("vfunfreeze AutoDL gates require CUDA.")
    return {
...
def _verify_alignment_permit(run_root: Path) -> tuple[str, str]:
    """Fail closed unless the registered frame-0 alignment gate passed."""

    permit_path = run_root / ALIGNMENT_PERMIT
    if not permit_path.is_file():
        raise FileNotFoundError(
            "The frame-0 paired alignment report is missing; Gate 3/4 stay sealed."
        )
    report = json.loads(permit_path.read_text(encoding="utf-8"))
    results = report.get("results", [])
    gains = {
        ("zero" if result.get("noise_seed") is None else str(result["noise_seed"])): float(
            result["paired_mae_gain"]
        )
        for result in results
    }
    zero_positive = gains.get("zero", float("-inf")) > 0.0
    positive_count = sum(1 for value in gains.values() if value > 0.0)
    if not zero_positive or positive_count < 3:
        raise ValueError(
            "The registered alignment criteria were not met; Gate 3/4 stay sealed "
            f"(zero_positive={zero_positive}, positive_count={positive_count})."
        )
    return ALIGNMENT_PERMIT, file_sha256(permit_path)


def _build_online_cuda_class():
    """Verbatim behavioral clone of the registered Zen CUDA policy class."""
    ...
    class _VfunfreezeOnlineCUDASmolVLA:
        def __init__(self, artifact, config, normalization, contract) -> None:
            if os.environ.get("ROSETTA_TORCH_DEVICE") != "cuda" or not torch.cuda.is_available():
                raise RuntimeError("vfunfreeze simulation requires the CUDA runtime.")
            self.device = torch.device("cuda")
            self.mixed_precision = str(config["mixed_precision"])
            if self.mixed_precision != "bf16":
                raise ValueError("vfunfreeze simulation mixed precision differs from its artifact.")
            ...
            if not isinstance(raw_action_space, dict):
                raise ValueError("vfunfreeze artifact has no explicit action-space identity.")
            ...
            if (
                ...
                raise ValueError("vfunfreeze artifact lost the bounded gripper decoder.")
            ...
            if file_sha256(contract_path) != str(config["action_contract_sha256"]):
                raise ValueError(
                    "vfunfreeze source and exported Action Contract checksums differ."
                )
        ...
        def predict(self, observation, instruction: str):
            ...
            if not isinstance(action, torch.Tensor) or tuple(action.shape) != expected_shape:
                raise ValueError("vfunfreeze simulator output differs from the Action Contract.")
            adapter_steps = [
                step
            ...
            if len(adapter_steps) != 1:
                raise ValueError("vfunfreeze simulator has no unique action decoder boundary.")
            ...
            if not isinstance(raw, torch.Tensor) or tuple(raw.shape) != expected_shape:
                raise ValueError("vfunfreeze simulator did not retain the pre-clipping action.")
            return raw[0].detach().cpu(), action[0].detach().cpu()

    return _VfunfreezeOnlineCUDASmolVLA


def _plan_path_from_argv() -> Path:
    if _ACTIVE_SIM_PLAN is not None:
        return _ACTIVE_SIM_PLAN
    raise RuntimeError("The vfunfreeze gate wrapper did not register its rendered sim plan.")


def _write_gate_selection(
    run_root: Path, selection_path: Path, artifact_dir: Path
    ...
    payload = {
        ...
        "hidden_test_loaded": False,
        "derived_from": {
            "vfunfreeze_selection_report": f"runs/{protocol.EXPERIMENT_ID}/selection/"
            f"{selection_path.name}",
            "vfunfreeze_selection_report_sha256": file_sha256(selection_path),
        },
    }
...
def _write_backup_evidence(run_root: Path, artifact_dir: Path) -> tuple[Path, str]:
    """Inventory-based backup evidence (registered Way precedent)."""
...
def main() -> int:
    ...
    if plan_id != protocol.VFU_PLAN_ID:
        raise ValueError("The gate wrapper only dispatches the vfunfreeze candidate.")
    ...
    if str(manifest["plan_id"]) != protocol.VFU_PLAN_ID:
        raise ValueError("The deploy artifact belongs to a different candidate plan.")
    if str(manifest["plan_sha256"]) != file_sha256(plan_path):
        raise ValueError("The deploy artifact predates the current candidate plan.")

    alignment_report, alignment_sha = _verify_alignment_permit(run_root)

    selection_source = (
        run_root / protocol.EXPERIMENT_ID / "selection" / f"{protocol.VFU_RUN_NAME}-selection.json"
    )
    ...
    sim_plan_id = f"{protocol.VFU_RUN_NAME}-sim-{REPORT_SUFFIX}"
    sim_code_blocks = "\n".join(
        ...
        def create_json(path: Path, payload: dict[str, Any]) -> None:
            if str(payload.get("gate", "")).startswith("m2_gate"):
                payload["vfunfreeze_protocol"] = {
                    ...
                    "alignment_permit_verified": True,
