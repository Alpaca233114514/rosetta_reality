"""Run fixed validation with the Aster plan and repaired action processors."""
...
DELEGATED_EVALUATOR_SHA256 = (
    "507422bf4baa90ec34e844e858cb386d7ea90c3f5faf3725f899bfacdbaac403"
)
...
def _checkpoint_source(
    ...
    if step not in allowed:
        raise ValueError("Checkpoint step is outside the Aster validation protocol.")
    ...
    if any(not path.is_file() or path.stat().st_size <= 0 for path in required):
        raise FileNotFoundError("Aster checkpoint files are incomplete.")
    ...
    if (
        ...
        raise ValueError("Aster checkpoint identity differs from its plan.")
