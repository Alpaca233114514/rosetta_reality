"""Export the selected candidate checkpoint as a gate-ready deploy artifact.

The artifact layout matches what the frozen simulation-gate engine loads
(``smolvla_sim_gate._load_artifact``) and what the executable offset-250
gradient gate consumes: ``pretrained_model/``, ``config.json`` (dataset
...
def _reload_validation(
    ...
    environment = {
        **os.environ,
        protocol.VCD_RELOAD_SOURCE_ENV: str(artifact_pretrained),
        protocol.VCD_VALIDATION_PREFIX_ENV: prefix_override,
    }
...
def main() -> int:
    ...
    artifact_id = f"{protocol.VCD_RUN_NAME}-step{step:04d}-deploy-001"
    ...
    artifact_config = {
        ...
        "training_only_treatment": {
            "visual_conditioning_profile": protocol.VCD_CONTRACT["profile"],
            "state_dropout_probability": protocol.VCD_CONTRACT["dropout_probability"],
            "state_dropout_training_only": True,
            "deployment_state_dropout": False,
        },
    ...
    preflight_report = (
        run_root / protocol.EXPERIMENT_ID / "preflight" / f"{protocol.VCD_PREFLIGHT_RUN_NAME}.json"
    )
    ...
    manifest = {
        ...
        "stage": "smolvla_vcdropout_export",
        "artifact_id": artifact_id,
        "experiment_id": protocol.EXPERIMENT_ID,
        "plan_id": plan_id,
