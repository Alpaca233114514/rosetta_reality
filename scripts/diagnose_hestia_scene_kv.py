...
from scripts.hestia_scene_kv import (  # noqa: E402
    CONDITIONS,
    SceneKV,
    condition_spec,
    validate_protocol,
...
def collect(plan, output):
    ...
    result = {
        ...
        "cross_checkpoint_donor": False,
        "intervention_applied": condition_spec(plan["condition"])[1] != "native",
...
def validate_permit(template, permit, condition, now):
    ...
    if not 0 < end - start <= 2400 or not start <= now < end:
...
def endpoint_controls(job, condition):
    ...
    for name in preceding:
        ...
        if (
            ...
            or control.get("intervention_applied") != (condition_spec(name)[1] != "native")
...
def main():
    ...
    if plan["id"] != "hestia-scene-kv-ablation-20260913-001":
    ...
    plan.update(
        ...
        reference_arrays=(
            ...
            if condition_spec(args.condition)[1] == "native"
            else str(job / f"base{base}_native" / "arrays.npz")
            if condition_spec(args.condition)[1] == "self"
            else None
        ),
