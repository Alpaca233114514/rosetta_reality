...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/faust-temporal-aggregation-003.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/faust-temporal-aggregation-003.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Faust temporal-aggregation status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=faust-temporal-aggregation-003 \
    scripts/run_m2_container.sh vla-sim-xpu \
    python -u scripts/smolvla_faust_temporal_aggregation_sim.py execution-diagnostic \
    --plan /workspace/configs/vla/smolvla_450m_aloha_insertion_faust_temporal_aggregation_sim_010.yaml \
    --seeds 10 \
    --maximum-steps 500 \
