...
from scripts.canonical_furnace import NAME, STAGES  # noqa: E402
...
def main():
    ...
    assert template["id"] == NAME and template["maximum_optimizer_steps"] == 5000
    assert template["stages"] == list(STAGES)
    for name, expected in template["sources"].items():
        assert sha(ROOT / name) == expected, name
    assert shutil.disk_usage(os.environ["ROSETTA_AUTODL_ROOT"]).free >= 5 * 1024**3
    ...
    except BaseException as exc:  # noqa: BLE001 - retain failure evidence before shutdown
