...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/aster-b8-gate3-003.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/aster-b8-gate3-003.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Aster Gate 3 status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=aster-b8-gate3-003 \
    scripts/run_m2_container.sh vla-sim-xpu \
    python -u scripts/smolvla_action_repair_sim_gate.py gate3 \
