...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/aster-b8-gate4-003.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/aster-b8-gate4-003.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Aster Gate 4 status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=aster-b8-gate4-003 \
    ...
    --gate3-report /workspace/runs/${EXPERIMENT_ID}/gates/gate3-smolvla-sim-003.json \
    >"$LOG_PATH" 2>&1
exit_code="$?"
set -e
