...
out=runs/iris-recovered-20260913-001
mkdir "$out"
ssh_registered() {
 /mnt/c/Windows/System32/OpenSSH/ssh.exe -o BatchMode=yes -o ConnectTimeout=15 \
  -p 20497 root@connect.cqa1.seetacloud.com "$@"
}
ssh_registered "cat '$workspace/runs/iris-k-scene-20260913-001/package.json'" > "$out/package.json"
size=$(ssh_registered "stat -c %s '$workspace/runs/iris-k-scene-20260913-001.tar'" | tr -d '\r')
[[ "$size" =~ ^[0-9]+$ && "$size" -le 5402263552 ]]
ssh_registered "cat '$workspace/runs/iris-k-scene-20260913-001.tar'" > "$out/archive.tar"
repo=$(wslpath -m "$PWD")
...
ssh_registered "test ! -e '$workspace/runs/iris-k-scene-20260913-001/transfer-receipt.json' && cat > '$workspace/runs/iris-k-scene-20260913-001/transfer-receipt.tmp' && mv '$workspace/runs/iris-k-scene-20260913-001/transfer-receipt.tmp' '$workspace/runs/iris-k-scene-20260913-001/transfer-receipt.json'" < "$out/verified/transfer-receipt.json"
