...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/way-state-jitter-smoke-002.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/way-state-jitter-smoke-002.log"
...
ROSETTA_CONTAINER_NAME=way-state-jitter-smoke-002 \
    scripts/run_m2_container.sh vla-xpu \
    python -u scripts/run_smolvla_state_robustness_smoke.py \
    --plan /workspace/configs/vla/smolvla_450m_aloha_insertion_way_state_jitter_smoke_002.yaml \
    >"$LOG_PATH" 2>&1
exit_code="$?"
set -e
