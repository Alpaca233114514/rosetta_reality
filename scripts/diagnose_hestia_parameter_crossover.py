...
from scripts.hestia_parameter_crossover import (  # noqa: E402
    CONDITIONS,
    condition_spec,
    patch_policy,
)
...
def collect(plan, output):
    ...
    intervention = None
    if plan["donor_checkpoint"] is not None:
        intervention = patch_policy(policy, Path(plan["donor_checkpoint"]) / "model.safetensors")
        with (output / "intervention.json").open("x") as stream:
            json.dump(intervention, stream, indent=2)
    policy.eval()
...
def validate_permit(template, permit, condition, now):
    ...
    if not 0 < end - start <= 900 or not start <= now < end:
...
def endpoint_controls(job, condition):
    ...
    for name in preceding:
        ...
        if (
            ...
            or control.get("intervention_applied") is not False
...
def main():
    ...
    if plan["id"] != "hestia-parameter-crossover-20260912-001":
    ...
    if (
        not profile
        or file_hash(profile) != plan["profile_sha256"]
    ...
    plan.update(
        ...
        donor_checkpoint=None if donor is None else str(sources[donor]),
        reference_arrays=plan["reference_endpoints"][str(base)] if donor is None else None,
