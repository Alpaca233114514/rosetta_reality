"""Run the registered fixed-validation protocol against the vfunfreeze plan.

The vision-front-end candidate's entry into the frozen fixed-validation engine
(``evaluate_smolvla_validation``), mirroring the proven vcdropout wrapper:

1. ``_validate_plan`` is replaced with the preregistered candidate identity
   checks (``smolvla_vfunfreeze_protocol.validate_vfunfreeze_plan``) plus an
...
3. The evaluation report gains a ``vfunfreeze_protocol`` provenance block
   binding the wrapper checksum, the candidate plan checksum and the
   substitution record.

Validation inputs stay clean: the vision front-end scope is installed only
inside the trainer process, never here — validated checkpoints reload with
the frozen-baseline flags exactly as they will at deployment.

Usage mirrors the frozen engine:

    python scripts/smolvla_vfunfreeze_validate.py --plan <candidate-plan> \
...
def _candidate_checkpoint_source(
    ...
    reload_source = os.environ.get(protocol.VFU_RELOAD_SOURCE_ENV)
...
def _compatible_view(plan: dict) -> dict:
    ...
    prefix = os.environ.get(protocol.VFU_VALIDATION_PREFIX_ENV) or (
        protocol.VFU_VALIDATION_PREFIX
    )
...
def _patch_engine(plan_path: Path, compatible: dict) -> None:
    ...
    def _create_json_with_protocol(path: Path, payload: dict) -> None:
        payload["vfunfreeze_protocol"] = {
            ...
            "plan_sha256": compatible["vfunfreeze_protocol"]["plan_sha256"],
            "prerequisite_scope": list(RUN_ROOT_SUBSTITUTIONS),
            "vision_front_end_scope_installed_in_validation": False,
...
def main() -> int:
    ...
    compatible["vfunfreeze_protocol"] = {
        "plan_sha256": file_sha256(plan_path),
        "plan_id": plan_id,
    }
    sys.argv = [
        "smolvla_vfunfreeze_validate",
        "--plan",
        str(plan_path),
        "--preflight-report",
