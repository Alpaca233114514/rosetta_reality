...
cd "$repo"
root=runs/hestia-prefix-receiver-20260912-001
output=runs/hestia-prefix-recovered-20260912-001
mkdir "$root"
failure() { printf 'receiver_failed_exit=%s\n' "$?" > "$root/failure.txt"; }
trap failure ERR
ssh -p "$port" "$host" "cd '$workspace' && python3 scripts/stream_hestia_prefix_results.py" \
...
docker.exe run --rm --network none --memory 2g --memory-swap 2g --cpus 2 \
  ...
  "$image" python scripts/verify_hestia_prefix_results.py \
  --archive "$root/results.tar" --output "$output/verified" --receipt "$root/receipt.json" \
  > "$root/verify.log" 2>&1
ssh -p "$port" "$host" "cd '$workspace' && python3 -c \"import json,pathlib,sys; data=json.load(sys.stdin); p=pathlib.Path('runs/hestia-prefix-path-20260912-001/transfer-receipt.json'); f=p.open('x'); json.dump(data,f); f.close()\"" \
