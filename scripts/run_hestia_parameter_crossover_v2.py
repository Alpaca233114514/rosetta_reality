"""Guarded four-condition reciprocal K/V intervention; requires explicit authorization."""
...
def supervise(args, template):
    ...
    registration = {
        ...
        "deadline_unix": started + 900,
        "shutdown_deadline_unix": started + 1200,
        "template_sha256": hashlib.sha256(args.template.read_bytes()).hexdigest(),
        "allowed_conditions": ["base1280", "base640", "base1280_kv640", "base640_kv1280"],
        ...
        for condition in registration["allowed_conditions"]:
            ...
            with (job / f"condition-{condition}.log").open("xb") as stream:
                child = subprocess.Popen(
                    [
                        sys.executable,
                        "scripts/diagnose_hestia_parameter_crossover_v2.py",
                        "--template",
                        str(args.template),
                        "--permit",
                        str(job / "permit.json"),
