"""Deadline-owned CUDA checkpoint observations; requires new explicit authorization."""
...
def supervise(args, template):
    ...
    registration = {
        ...
        "deadline_unix": started + 480,
        "shutdown_deadline_unix": started + 600,
    ...
    try:
        ...
        check = subprocess.run(
            [
                ...
                "-p",
                "no:cacheprovider",
                "tests/test_hestia_prefix_path.py",
                "tests/test_hestia_prefix_supervisor.py",
                "tests/test_hestia_local_checkpoint.py",
                "tests/test_hestia_prefix_transfer.py",
            ],
            capture_output=True,
            text=True,
            timeout=45,
        ...
        for step in registration["allowed_steps"]:
            ...
            with (job / f"step-{step:06d}.log").open("xb") as stream:
                child = subprocess.Popen(
                    [
                        sys.executable,
                        "scripts/diagnose_hestia_prefix_path.py",
                        "--template",
                        str(args.template),
                        "--permit",
                        str(job / "permit.json"),
                ...
                active = {"pid": child.pid, "ticks": ticks(child.pid), "step": step}
                ...
                if status != 0:
                    raise RuntimeError(f"Checkpoint {step} collector exit {status}")
        ...
        if sum(v["bytes"] for v in files.values()) > 64 * 1024**2:
        ...
        stop = min(registration["shutdown_deadline_unix"], time.time() + 120)
