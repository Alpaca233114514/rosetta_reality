...
set -Eeuo pipefail
cd "$1"
test ! -e runs/iris-002-gate34-20260913-001
test ! -e runs/iris-002-gate34-20260913-001-supervisor.log
test "$(sha256sum "$2" | cut -d' ' -f1)" = "$3"
mkdir -p runs
nohup bash scripts/launch_iris_gate_worker.sh "$2" "$3" \
 >runs/iris-002-gate34-20260913-001-supervisor.log 2>&1 </dev/null &
