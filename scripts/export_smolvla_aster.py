"""Export Aster while preserving and independently reloading its repaired action boundary."""
...
import evaluate_smolvla_aster_validation as repair_evaluator  # noqa: E402
...
def _plan_path() -> Path:
    try:
        ...
        raise ValueError("Aster export requires an explicit --plan path.") from error
...
def _copy_policy(source: Path, destination: Path) -> None:
    ...
    for pipeline_name in pipeline_names:
        ...
        for step in steps:
            ...
            if state_path.is_absolute() or ".." in state_path.parts or len(state_path.parts) != 1:
...
def main() -> int:
    ...
    cache_root = run_root / "compiler_cache" / f"aster-export-{file_sha256(plan_path)[:12]}"
    ...
    _plan, _base_path, experiment = formal_runner._validate_plan(plan_path)
    ...
    def create_json(path: Path, payload: dict[str, Any]) -> None:
        if path.name in {"config.json", "manifest.json"}:
            payload["action_space"] = action_space.as_dict()
            payload["bounded_gripper_decoder"] = True
            payload["temporal_loss_profile"] = "first_action_only"
            payload["temporal_loss_normalization"] = _plan["loss_contract"][
                "normalization"
            ]
            payload["action_repair_export_script_sha256"] = file_sha256(Path(__file__))
