...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/aster-training-pose-diagnostic-001.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/aster-training-pose-diagnostic-001.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Aster training-pose status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=aster-training-pose-diagnostic-001 \
    scripts/run_m2_container.sh vla-sim-xpu \
    python -u scripts/smolvla_action_repair_sim_gate.py execution-diagnostic \
    --plan /workspace/configs/vla/smolvla_450m_aloha_insertion_aster_sim_003.yaml \
    --seeds 10 \
    --maximum-steps 500 \
