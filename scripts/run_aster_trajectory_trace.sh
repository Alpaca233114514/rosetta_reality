...
readonly STATUS_PATH="${ORCHESTRATION_ROOT}/aster-trajectory-trace-001.status"
readonly LOG_PATH="${ORCHESTRATION_ROOT}/aster-trajectory-trace-001.log"

mkdir -p -- "$ORCHESTRATION_ROOT"
[[ ! -e "$STATUS_PATH" && ! -e "$LOG_PATH" ]] \
    || { printf 'error: Aster trajectory trace status or log already exists\n' >&2; exit 3; }
...
ROSETTA_CONTAINER_NAME=aster-trajectory-trace-001 \
    scripts/run_m2_container.sh vla-sim-xpu \
    python -u scripts/diagnose_smolvla_aster_trajectory.py \
    --plan /workspace/configs/vla/smolvla_450m_aloha_insertion_aster_trace_sim_006.yaml \
    --episode 2 \
    --seed 10 \
