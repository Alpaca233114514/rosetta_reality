...
def validate_manifest(value):
    ...
    pattern = re.compile(
        ...
        r"|condition-base(?:640|1280)\.log|base(?:640|1280)/"
        r"(?:arrays\.npz|result\.json|failure\.json|control-failure\.npz|execution-count\.json"
        r"|attention\.jsonl|sentinel-(?:0|40)-[0-9]-(?:1|3|5|7|9|11|13|15)\.npz))"
    )
    total = 0
    for name, item in value["files"].items():
        ...
        total += item["bytes"]
    if total != value.get("total_bytes") or not 0 < total <= 512 * 1024**2:
...
def main():
    ...
    if args.archive.stat().st_size > 513 * 1024**2:
