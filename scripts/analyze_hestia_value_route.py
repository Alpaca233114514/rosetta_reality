...
from scripts.hestia_value_route import CONDITIONS, component_mode

IMAGE = "sha256:fb3c1bbda42881fac9b7725d3acb436119934f0c60af29747339d5951c3039da"
ROOT = Path("runs/hestia-value-route-recovered-20260912-001/verified")
OUTPUT = Path("runs/hestia-value-route-analysis-20260912-001/verified")
...
def main():
    ...
    receipt = read_json("runs/hestia-value-route-receiver-20260912-001/receipt.json")
    ...
    template = read_json("reports/training/m2-smolvla-hestia-value-route-template-2026-09-12.json")
    ...
    for name in CONDITIONS:
        ...
        require(intervention == r["value_route"], "Intervention report differs")
        ...
        require(not intervention["labels_used"], "Label leakage")
        require(intervention["layout"]["shape"] == [1, 241, 320], "Prefix layout differs")
        if component_mode(name) != "native":
            base_name = "base1280" if name.startswith("base1280") else "base640"
            native_record = read_json(ROOT / base_name / "intervention.json")
            require(intervention["same_base_identity_matched"], "Same-base identity failed")
            require(intervention["rows"] == native_record["rows"], "Native input/output drift")
            require(
                intervention["native_reference_sha256"]
                == file_hash(ROOT / base_name / "intervention.json"),
                "Native reference seal changed",
            )
    ...
    previous_receipt = read_json("runs/hestia-kv-split-receiver-20260912-003/receipt.json")
    ...
    for name in ("base1280", "base640", "base1280_v640", "base640_v1280"):
        ...
        with np.load(p, allow_pickle=False) as saved:
            ...
            if name in CONDITIONS[:2]:
                ...
                current = "base1280_vfull640" if name == "base1280_v640" else "base640_vfull1280"
                for key in ("normalized_predictions", "standard_predictions"):
                    require(
            ...
            for metric in errors[CONDITIONS[0]]:
                ...
                out[metric] = {
                    ...
                    "causal": {
                        ...
                        for kind in ("full", "image", "language", "state")
                    },
    ...
    native_rows = {
        name: read_json(ROOT / name / "intervention.json")["rows"] for name in CONDITIONS[:2]
    }
    route_identity = {
        str(layer): {
            route: [
                native_rows["base1280"][f"{row}:{layer}"]["input_routes"][route]
                == native_rows["base640"][f"{row}:{layer}"]["input_routes"][route]
                for row in range(45)
            ]
            for route in ("image", "language", "state")
        }
    ...
    result = {
        ...
        "results": results,
        "cross_checkpoint_input_equal_by_layer_route_row": route_identity,
