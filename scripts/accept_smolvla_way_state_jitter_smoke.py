"""Accept the plan-bound Way state-jitter optimizer smoke without loading weights."""
...
import run_smolvla_state_robustness_smoke as smoke_runner  # noqa: E402
...
def _trackio_evidence(
    ...
    try:
        ...
        connection.close()
    if len(configs) != 1:
        raise ValueError("Way smoke must have exactly one durable Trackio config.")
    ...
    if mismatches:
        raise ValueError(f"Way smoke Trackio config differs from its plan: {mismatches}")
    if {str(row[0]) for row in rows} != {run_id}:
        raise ValueError("Way smoke Trackio rows have an ambiguous identity.")
    ...
    for _, step, raw in rows:
        ...
        if "train/loss" in metrics:
            ...
            if step_number in train:
                raise ValueError("Way smoke contains duplicate train metrics.")
            for key in (
                "train/loss",
                "train/grad_norm",
                "train/lr",
    ...
    if set(train) != expected_steps or checkpoint_steps != expected_steps:
        raise ValueError("Way smoke lacks a registered metric or checkpoint step.")
...
def _checkpoint_evidence(
    ...
    for step in range(1, int(smoke["steps"]) + 1):
        ...
        if any(not path.is_file() or path.stat().st_size <= 0 for path in required):
            raise FileNotFoundError("Way smoke checkpoint is incomplete.")
        ...
        if (
            ...
            raise ValueError("Way smoke checkpoint identity differs from the plan.")
        evidence.append(
            {
                "step": step,
...
def main() -> int:
    ...
    plan, _control, base_path, experiment = smoke_runner._validate_plan(plan_path)
    smoke = plan["optimizer_smoke"]
    ...
    if trackio["peak_xpu_within_guardrail"] is not True:
        raise MemoryError("Way state-jitter smoke exceeded its XPU guardrail.")
    contract_path = REPOSITORY_ROOT / str(experiment["action_contract"]["derived"])
    report = {
        "schema_version": 1,
        "status": "passed",
        "stage": "smolvla_state_robustness_optimizer_smoke_acceptance",
    ...
    if destination.exists():
        raise FileExistsError("Way state-jitter smoke acceptance is create-only.")
