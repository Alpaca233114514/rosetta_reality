...
from scripts.hestia_kv_split import (  # noqa: E402
    CONDITIONS,
    condition_spec,
    patch_policy,
)
...
def collect(plan, output):
    ...
    if plan["donor_checkpoint"] is not None:
        intervention = patch_policy(
            policy, Path(plan["donor_checkpoint"]) / "model.safetensors", plan["condition"]
        )
...
def validate_permit(template, permit, condition, now):
    ...
    if not 0 < end - start <= 1200 or not start <= now < end:
...
def main():
    ...
    if plan["id"] != "hestia-kv-split-20260912-001":
