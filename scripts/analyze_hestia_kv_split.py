"""Analyze K-only/V-only causal conditions from verified saved arrays only."""
...
from scripts.hestia_kv_split import CONDITIONS, keys_for

IMAGE = "sha256:fb3c1bbda42881fac9b7725d3acb436119934f0c60af29747339d5951c3039da"
ROOT = Path("runs/hestia-kv-split-recovered-20260912-003/verified")
OUTPUT = Path("runs/hestia-kv-split-analysis-20260912-001/verified")
...
def causal(values, kind):
    a, b, c, d = (
        values[name]
        for name in ("base1280", "base640", f"base1280_{kind}640", f"base640_{kind}1280")
    )
...
def main():
    ...
    receipt = read_json("runs/hestia-kv-split-receiver-20260912-003/receipt.json")
    ...
    template = read_json("reports/training/m2-smolvla-hestia-kv-split-template-2026-09-12.json")
    ...
    previous = Path("runs/hestia-parameter-crossover-v2-recovered-20260912-002/verified")
    previous_receipt = read_json(
        "runs/hestia-parameter-crossover-v2-receiver-20260912-002/receipt.json"
    )
    ...
    for name in ("base1280", "base640", "base1280_kv640", "base640_kv1280"):
        ...
        with np.load(p, allow_pickle=False) as saved:
            ...
            if name in CONDITIONS[:2]:
                ...
                predictions[name] = saved["standard_predictions"].astype(np.float64)
        ...
        for window, slots in (("full", slice(0, 50)), ("first", slice(0, 1))):
            ...
            for metric in errors[CONDITIONS[0]]:
                ...
                out[metric] = {
                    ...
                    "causal": {kind: causal(values, kind) for kind in ("k", "v", "kv")},
                    "kv_nonadditivity": {
                        "base1280": summarize(
                            values["base1280_kv640"]
                            - values["base1280_k640"]
                            - values["base1280_v640"]
                            + values["base1280"]
                        ),
                        "base640": summarize(
                            values["base640_kv1280"]
                            - values["base640_k1280"]
                            - values["base640_v1280"]
                            + values["base640"]
                        ),
                    },
