...
def prepare():
    ...
    assert (
        digest(incoming / "000256/pretrained_model/model.safetensors")
        == template["arms"]["A"]["checkpoint_files"]["model.safetensors"]
    )
    cp = (
    ...
    manifest = {
        ...
        "A_checkpoint_files": inventory(incoming / "000256"),
        "shutdown_authorized": True,
        "authorization": (
            "User authorized task-based computation, unattended server execution, "
        ...
        "order_note": (
            "A input/sampler contract precedes smoke; A new train8 fit is checked before B."
        ),
    }
...
def wrapped():
    ...
    def monitor():
        while not stop.wait(1):
            if (
                torch.cuda.max_memory_allocated() > 8 * 1024**3
    ...
    try:
        ...
        stop.set()
        result = {
            "status": "passed" if passed and not reasons else "failed",
...
def shutdown():
    ...
    assert not sessions.stdout.strip() and sessions.returncode in (0, 1), (
        "Unrelated tmux session present"
    )
    for proc in Path("/proc").iterdir():
        ...
        try:
            ...
            continue
        assert not (
            any(b"rosetta" in x for x in argv)
            and any(
                any(
                    t in x
                    for t in (
        ...
        ), "Unrelated project worker present"
    save(
        JOB / "shutdown-request.json",
        {
            "time": time.time(),
            "shutdown_wrapper_sha256": expected,
            "instance_release_requested": False,
            "platform_billing_independently_verified": False,
        },
    )
    os.sync()
...
def worker():
    ...
    template.update(
        ...
        started_unix=watchdog["started_unix"],
    ...
    try:
        ...
        run(
            ...
            [
                ...
                "pytest",
                "-q",
                "tests/test_smolvla_visual_coverage.py",
                "tests/test_smolvla_fixed_visual_samples.py",
                "tests/test_smolvla_tracking_composition.py",
                "tests/test_smolvla_v2_error_boundaries.py",
            ],
        )
        run(
            "data-check",
        ...
        accept(
            ...
            {
                "status": "passed",
                "doctor_log_sha256": digest(JOB / "doctor.log"),
                "test_logs": {
                    n: digest(JOB / (n + ".log")) for n in ("static-tests", "data-check")
                },
        ...
        accept(
            "resource_preflight",
            {
                "status": "passed",
                "reports": {
                    name: evidence(JOB / f"{name}-resources.json")
                    for name in ("preflight-b1", "preflight-b4")
                },
            },
        )
        ...
        accept(
            ...
            {
                "status": "passed",
                "samples": evidence(JOB / "sample-contract.json"),
        ...
        run(
            "smoke-update-audit",
            [sys.executable, old_check, smoke_plan, str(JOB / "smoke-update-audit.json")],
        )
        ...
        assert (
            load(JOB / "smoke-first.npz.json")["pid"] != load(JOB / "smoke-reload.npz.json")["pid"]
        )
        accept(
            "two_step_smoke_reload",
            {
                "status": "passed",
                "full_chunk_exact": True,
                "smoke_audit": evidence(JOB / "smoke-update-audit.json"),
                "predictions_sha256": [digest(JOB / f"smoke-{s}.npz") for s in ("first", "reload")],
            },
        )
        native_a("control-reproduction")
        reproduced, historical = (
            load(JOB / "control-reproduction.json"),
            load(ROOT / "runs/visual-native-small-001/pilot-eval-003.json"),
        )
        for key in (
            "source_sha256",
            "episodes",
            "image_sha256",
            "results",
            "native_denoising_steps",
        ):
            assert reproduced[key] == historical[key], (
                "Historical control reproduction differs: " + key
            )
        accept(
            "control_reproduction",
            {
                "status": "passed",
                "exact_saved_values": True,
                "reproduction": evidence(JOB / "control-reproduction.json"),
            },
        )
        ...
        run(
            ...
            heavy=True,
            minimum_remaining=720,
        )
        run(
            "B-update-audit",
            [sys.executable, old_check, main_plan, str(JOB / "B-update-audit.json")],
        )
        cp = checkpoint("main256")
        state = load(cp / "training_state/scheduler_state.json")
        assert (
            state["last_epoch"] == 256
            and state["_step_count"] == 257
            and state["_last_lr"] == [2.5e-6]
        )
        ...
        for index, values in enumerate(resources["learning_rates"]):
            factor = (
                ((1 / 17 - 1) * (1 - index / 16) + 1)
                if index < 16
                else (0.975 * 0.5 * (1 + math.cos(math.pi * index / 256)) + 0.025)
            )
            assert len(values) == 1 and math.isclose(
                values[0], 1e-4 * factor, rel_tol=1e-12, abs_tol=1e-16
            )
        samples, schedule = (
            load(JOB / "sample-contract.json"),
            load(JOB / "sampler.json")["arms"]["B"]["schedule"],
        )
        mapping = dict(
            zip(samples["actual_train_episodes"], samples["actual_train_view_indices"], strict=True)
        )
        assert resources["sampled_indices"] == [mapping[episode] for episode in schedule], (
            "Actual 1024 sample order differs"
        )
        ...
        control_cp = (
            Path(os.environ["ROSETTA_CHECKPOINT_ROOT"])
            / template["arms"]["A"]["checkpoint_relative_to_root"]
        )
        ...
        accept(
            ...
            {
                "status": "passed",
                "update_audit": evidence(JOB / "B-update-audit.json"),
                "actual_optimizer_and_sampler": evidence(JOB / "main256-resources.json"),
                "checkpoint_files": inventory(cp),
            },
        )
        ...
        for arm in ("A", "B"):
            ...
            run(
                ...
                [
                    ...
                    str(JOB / f"{arm}-first"),
                    "--second",
                    str(JOB / f"{arm}-reload"),
                    "--output",
                    str(JOB / f"{arm}-reload-check.json"),
                ],
            )
        run(
            "compare-arms",
        ...
        save(
            JOB / "closure.json",
            {
                "status": "completed",
                "last_stage": stage,
                "comparison": load(JOB / "comparison.json"),
                "m2_complete": False,
