...
from scripts.diagnose_hestia_v_layer import endpoint_controls, validate_permit
from scripts.hestia_parameter_crossover import kv_keys
from scripts.hestia_v_layer import CONDITIONS, keys_for, substitute
from scripts.verify_hestia_v_layer_results import validate_manifest


def test_single_v_layer_partition_and_reciprocal_identity():
    groups = [set(keys_for(CONDITIONS[i])) for i in range(2, 18, 2)]
    assert len(groups) == 8 and all(len(g) == 1 for g in groups)
    assert set.union(*groups) == {k for k in kv_keys() if ".v_proj." in k}
    for i in range(2, 18, 2):
        assert keys_for(CONDITIONS[i]) == keys_for(CONDITIONS[i + 1])


@pytest.mark.parametrize("condition", CONDITIONS[2:])
def test_only_registered_layer_changes(condition):
...
def test_eighteen_conditions_and_fortyfive_minute_work_limit():
    permit = {
    ...
    permit["deadline_unix"] = 2801
    with pytest.raises(ValueError, match="deadline"):
        validate_permit("abc", permit, CONDITIONS[-1], 101)


def test_transfer_accepts_registered_eighteen_arms_only():
    files = {
