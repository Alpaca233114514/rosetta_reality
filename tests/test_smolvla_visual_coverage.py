...
DIMS = [{"name": "joint", "unit": "radian", "minimum": -10, "maximum": 10},
        {"name": "gripper", "unit": "normalized", "encoding": "0_closed_1_open",
         "minimum": 0, "maximum": 1}]


def evidence(arm="A"):
    target = np.repeat(np.array([[-1, 0], [1, 1], [0, .5], [-.5, .25], [.5, .75]],
    ...
    arrays = {"normalized_predictions": np.repeat(target[None], 4, axis=0),
              "standard_predictions": np.repeat(target[None], 4, axis=0),
              "internal_grippers": np.zeros((4, 5, 3, 1)),
              "normalized_targets": target.copy(), "standard_targets": target.copy(),
              "valid_mask": np.ones_like(target, dtype=bool), "noise": noise}
    meta = {"protocol": PROTOCOL, "arm": arm, "episodes": [10, 11, 12, 20, 21],
            "views": {"train8": [10, 11], "train40": [10, 11, 12], "dev5": [20, 21]},
            "hidden_episodes": [30], "hidden_test_loaded": False, "target_conditioning": False,
            "noise_conditions": [None, 20260905, 20260906, 20260907],
            "noise_hashes": [array_hash(n) for n in noise], "chunk_length": 3,
            "max_action_dim": 4, "native_denoising_steps": 10, "model_mode": "eval_inference",
            "dimensions": copy.deepcopy(DIMS), "nonvisual_hashes": ["a" * 64] * 5,
            "image_hashes": [f"{i:064x}" for i in range(5)],
            "common_identity": {"registration": "synthetic"}, "arm_identity": {"arm": arm},
            "process": {"pid": 1, "invocation_id": "first"}}
    return arrays, meta


def test_all_nonself_averages_errors_not_predictions():
    target = np.repeat(np.array([[-2., 0], [0, .5], [2, 1]])[:, None], 2, axis=1)
...
def test_gripper_failure_not_hidden_by_joint_fit():
    arrays, meta = evidence()
    arrays["normalized_predictions"][:, 3:, :, 1] = .5
...
def test_reload_compares_later_chunk_values_even_if_first_action_equal(tmp_path, changed):
    arrays, meta = evidence()
    write_bundle(tmp_path / "first", arrays, meta)
    other = copy.deepcopy(meta)
    other["process"] = {"pid": 2, "invocation_id": "second"}
    if changed:
        arrays["normalized_predictions"][0, 0, 2, 0] += .01
...
def test_perfect_both_arms_does_not_prove_coverage_gain():
    ...
    assert not result["coverage_gain_passed"] and not result["offline_metric_criteria_passed"]
...
def test_coverage_gain_needs_fit_and_physical_nonregression():
    a, am = evidence("A")
    b, bm = evidence("B")
    a["normalized_predictions"][:, 3:] += .3
    a["standard_predictions"][:, 3:] += .3
...
def test_draft_refuses_execution_before_model_import(monkeypatch):
    ...
    with pytest.raises(ValueError, match="draft cannot run"):
        module.check_execution({"protocol": PROTOCOL, "status": "draft_no_compute_authorization"})
...
def test_plan_changes_active_scope_and_preserves_native_recipe():
    ...
    assert sampler["sample_identities"] == [{"episode": e, "frame": 0} for e in review["data"]["train40"]]
...
def test_real_loader_rejects_pending_candidate_authorization():
    ...
    with pytest.raises(ValueError, match="preregistered"):
        validate_plan_structure(candidate, known_features={f["name"] for f in candidate["features"]})
