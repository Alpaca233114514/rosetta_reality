...
from scripts import diagnose_hestia_image_value_component as fixed
from scripts import diagnose_hestia_parameter_crossover as old
from scripts.hestia_image_value_component import CONDITIONS, component_mode, condition_spec
...
def fixture(tmp_path, monkeypatch, module, condition="base1280", *, fake_gpu=True):
    ...
    plan = {
        ...
        if module is old
        else "hestia-image-value-component-20260912-001",
        "output": "job",
...
def test_full_donor_positive_control_failure_blocks_components(tmp_path, monkeypatch):
    calls, _ = fixture(tmp_path, monkeypatch, fixed, "base1280_vglobal640")
    path = tmp_path / "job/base1280_vfull640/result.json"
