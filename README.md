...
450M**. M2 remains incomplete; offline metrics and safety checks do not establish
closed-loop task success. Research reports remain in the local archive.
...
reuse and gate design. Required executable protocol configurations live in
configs/. Historical evidence references are local-only and fail closed when
...
M1 is complete for its bounded acceptance slice. M2 is not complete;
core infrastructure and offline checks do not establish closed-loop success.

Research reports, run results and local operations are excluded from this
source distribution. Historical protocols may reference locally retained
evidence; absence of that evidence must not bypass their admission checks.
See [the SmolVLA architecture](docs/m2-smolvla-architecture.md) for code ownership.
