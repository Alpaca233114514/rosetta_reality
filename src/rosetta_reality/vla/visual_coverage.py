...
def action_groups(dimensions: list[dict]) -> dict[str, list[int]]:
    ...
    groups = {
        "all_valid": list(range(len(names))),
        "joint_radian": [],
        "gripper_normalized": [],
    }
    for index, d in enumerate(dimensions):
        ...
        if (
            not np.isfinite([d["minimum"], d["maximum"]]).all()
...
def score_view(prediction, target, valid_mask, dimensions: list[dict]) -> dict:
    ...
    if n < 2 or d != len(dimensions):
        raise ValueError(
            "Need at least two scenes and exactly the valid action dimensions."
        )
    if mask.dtype != np.bool_ or mask.shape != y.shape or not mask.all():
        raise ValueError(
            "Frame-zero protocol requires complete identical valid chunks."
        )
    result = {}
    for name, indices in groups.items():
        ...
        correct, wrong, floor = map(
            float, (per_correct.mean(), per_wrong.mean(), per_floor.mean())
        )
        ...
        result[name] = {
            ...
            "mean_floor": floor,
            "epsilon": epsilon,
            "visual_gain": wrong - correct,
            "below_mean_gain": floor - correct,
            "measurable": measurable,
            "status": "measured" if measurable else "not measurable",
            "passed": bool(
                measurable and wrong - correct > epsilon and floor - correct > epsilon
            ),
...
def standard_diagnostics(prediction, target, dimensions: list[dict]) -> dict:
    p, y = (
        _finite(prediction, "Standard prediction"),
        _finite(target, "Standard target"),
    )
    ...
    for horizon, sl in (("chunk", slice(None)), ("first_action", slice(0, 1))):
        ...
        result[horizon] = {
            name: {
                "mae": float(np.abs(error[..., indices]).mean()),
        ...
        result[horizon]["per_dimension_mse"] = (
            np.square(error).mean(axis=(0, 1)).tolist()
        )
    return result
...
def validate_bundle(arrays: dict[str, np.ndarray], metadata: dict) -> None:
    ...
    if (
        ...
        or metadata.get("target_conditioning") is not False
    ):
        raise ValueError("Hidden access or teacher-forced conditioning is forbidden.")
    if (
        metadata.get("native_denoising_steps") != 10
        or metadata.get("model_mode") != "eval_inference"
    ):
    ...
    if any(type(ep) is not int for ep in episodes) or len(set(episodes)) != len(
        episodes
    ):
    ...
    if not set(views["train8"]) <= set(views["train40"]) or set(views["train40"]) & set(
        views["dev5"]
    ):
    ...
    if arrays["internal_grippers"].shape != (
        *shape[:-1],
        len(groups["gripper_normalized"]),
    ):
    ...
    if len(set(metadata["image_hashes"])) != len(episodes):
        raise ValueError(
            "Image duplicate found; preserve and review the registered scope."
        )
    ...
    if type(metadata["process"].get("pid")) is not int or not metadata["process"].get(
        "invocation_id"
    ):
...
def summarize_bundle(arrays: dict[str, np.ndarray], metadata: dict) -> dict:
            ...
            conditions.append(
                {
                    ...
                    "normalized": score_view(
                        arrays["normalized_predictions"][noise, indices], y, mask, dims
                    ),
                    "standard": standard_diagnostics(
                        arrays["standard_predictions"][noise, indices], ys, dims
                    ),
                    "gripper_internal_outside_support_rate": float(
                        (np.abs(g) > np.pi / 2).mean()
                    ),
                }
            )
    ...
    return {
        "views": results,
        "train40_mean_on_dev5_mse": baseline,
        "training_fit_conditions": fit,
        "training_fit_passed": fit >= 3,
        "development_conditions": dev_pass,
        "development_passed": sum(dev_pass) >= 3,
        "m2_complete": False,
...
def write_bundle(
    directory: Path, arrays: dict[str, np.ndarray], metadata: dict
) -> None:
    validate_bundle(arrays, metadata)
...
def compare_arms(a: dict, am: dict, b: dict, bm: dict) -> dict[str, Any]:
    ...
    for ac, bc in zip(ar["views"]["dev5"], br["views"]["dev5"], strict=True):
        ...
        gains.append(
            y["correct"] < x["correct"] - epsilon
            and y["visual_gain"] > x["visual_gain"] + epsilon
        )
    nonregression = {}
    for horizon in ("chunk", "first_action"):
        for group in ("joint_radian", "gripper_normalized"):
            old = float(
                np.mean(
                    [v["standard"][horizon][group]["mse"] for v in ar["views"]["dev5"]]
                )
            )
            new = float(
                np.mean(
                    [v["standard"][horizon][group]["mse"] for v in br["views"]["dev5"]]
                )
            )
            nonregression[f"{horizon}/{group}"] = new <= old + 1e-8 * max(
                1.0, abs(old), abs(new)
            )
    return {
        "A": ar,
        "B": br,
        "coverage_gain_conditions": gains,
        "coverage_gain_passed": sum(gains) >= 3,
        "physical_nonregression": nonregression,
        "offline_metric_criteria_passed": bool(
            ar["training_fit_passed"]
            and br["training_fit_passed"]
            and br["development_passed"]
            and sum(gains) >= 3
